...
export default class WSClient {
  constructor (props) {
    const _this = this;
    const { wsServerAddresses, onMessage, onOpen, onClose, onError, onChangeConnectionState } = props;
    l.debug('wsServerAddresses ', wsServerAddresses);
    _this.wsServerAddresses = wsServerAddresses;
    _this.wsSocket = null;
    ...
    _this.wsServerAddress = null;
    _this.connect();
  }

  connect () {
    const _this = this;
    _this.wsServerAddress = _this.getNextTryConnWsServerAddress();
  ...
  getNextTryConnWsServerAddress () {
    const _this = this;
    l.debug('current wsServerAddress ', _this.wsServerAddress);
    l.debug('getNetxtTryConnWsServerAddresses ', _this.wsServerAddresses);
    if (_this.wsServerAddress) {
      _this.wsServerAddresses.push(_this.wsServerAddress);
      return _this.wsServerAddresses.shift();
    } else {
      return _this.wsServerAddresses.shift();
    }
  }
