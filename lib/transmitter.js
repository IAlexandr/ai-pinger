...
export default {
  ...
  createWSServer () {
    ...
    this.getRegHosts((err, regHosts) => {
      ...
      _this.WSServer = new WSServer({
        ...
        regHosts,
        onConnect: (client) => {
          console.log('wsserver client send state:', _this.state);
          client.send(JSON.stringify(_this.state));
        },
        onMessage: (msg) => {
          console.log('!!!incomming message.. ', JSON.parse(msg));
  ...
  onPingersStateChange (newState) {
    ...
    this.WSServer.broadcast(this.state);
    console.log('Pingers: state changed.', this.state);
    l.log('wsserver broadcated new state.');
  }
};
