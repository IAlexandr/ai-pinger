...
export default {
  ...
  createWSServer () {
    ...
    this.getRegHosts((err, regHosts) => {
      ...
      _this.WSServer = new WSServer({
        ...
        regHosts,
        onConnect: (client) => {
          console.log('transmitter.onConnect client.send (stringify): ', _this.state);
          client.send(JSON.stringify(_this.state));
        },
        onMessage: (msg) => {
          console.log('transmitter.onMessage: incomming message (parse).. just console', JSON.parse(msg));
        }
      });
    });
  },
  ...
  onPingersStateChange (newState) {
    ...
    l.log('transmitter.onPingersSateChange: this.WSServer.broadcast newState (stringify) ', JSON.stringify(this.state));
  }
};
