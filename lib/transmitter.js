  ...
  initializeAppRequesters () {
    ...
    if (typeof app === 'object' && Object.keys(app).length > 0) {
      Object.keys(app).forEach((appName) => {
        _this.apps[appName] = new Requester(
          {
            ...app[appName],
            ...{
              onChangeState: (newState) => {
                _this.state.apps[appName] = newState;
                l.debug('transmitter.onRequesterStateChange: _this.state.requesters (stringify) ', JSON.stringify(_this.state.apps));
                _this.onChangeState();
              }
            }
          });
